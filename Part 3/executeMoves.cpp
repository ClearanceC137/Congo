...
string generate_fen(string state[]){
	string fen;
	return fen;
}
int getrow(char c){
...
void makeMove(string state,string move){
	...
	if(b->grid[6-movetorow][movetocolumn]=="l" || b->grid[6-movetorow][movetocolumn]=="L"  ){
		if(b->side_to_play =="b"){
			update ="Black wins";	
		}
		else{
	...
	if( (b->grid[6-movefromrow][movefromcolumn]=="p" && (6-movetorow) == 6 ) || (b->grid[6-movefromrow][movefromcolumn]=="P"  && (6-movetorow) == 0 )){
			/** pawn changed into superpawn  **/
		if(b->side_to_play =="b"){
			b->grid[6-movefromrow][movefromcolumn]="s";
		}else{
			b->grid[6-movefromrow][movefromcolumn]="S";				
		}			
	}

	b->grid[6-movetorow][movetocolumn]=b->grid[6-movefromrow][movefromcolumn]; 
	b->grid[6-movefromrow][movefromcolumn]="";
	temp = 	b->grid[6-movetorow][movetocolumn];
	...
	if(6-movetorow == 3 && 6-movefromrow != 3 ){
		...
		b->grid[6-movetorow][movetocolumn] = temp;
	}
	/* ---- generate fen sting  -------*/
...
int main(){
	...
	for(int i = 0  ; i < N ; i++){
		makeMove(states[i] ,moves[i]);
		if(i != N-1){
			cout<<endl;			
		}
	} 	
//	string result,temp,update;
//	board *b = new movepiece("1z5/pPp1lP1/6p/4P1g/4L1p/2p2pP/7 b 12 ");
//	string move = "g4e4";
//	int movefromrow = getrow(move[1]);
//	int movefromcolumn =getcolumn(move[0]);
//	
...
//	}
//	/* ---- make move ---- */
//	if( (b->grid[6-movetorow][movetocolumn]=="p" && (6-movetorow) == 6 ) || (b->grid[6-movetorow][movetocolumn]=="P"  && (6-movetorow) == 0 )){
//			/** pawn changed into superpawn  **/
//		if(b->side_to_play =="b"){
//			b->grid[6-movefromrow][movefromcolumn]="s";
//		}else{
