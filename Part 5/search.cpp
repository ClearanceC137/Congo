...
class square{
	public:
		...
		int rank,col;
		int value;
		square(string color,char type , int rank,int col,int value){
			...
			for(int i = 0 ; i < 7 ; i++){
					...
					for(int j = 0 ; j < strSize ; j++){
						...
						if(toupper(type) == curr[j]){
							square* block = new square("white",type, 7-i,index-1,value);
							p.push_back(*block);
						}
						if(type == curr[j]){
							square* block = new square("black",type, 7-i,index-1,value);
							p.push_back(*block);
						}						
					}	
			}
...
int makeMove(string state,string move,string &nextState){
    int attack = 0;
...
bool isGameOver(string board[7][7])
{
    bool foundL = false;
    bool foundl = false;
    for (int r = 0; r < 7; r++)
		...
		int evaluation(board *b){
		    ...
		    if (rscore != 10000 && rscore != -10000 ){
				...
				for(vector<string> v : moves){
					for(string move : v){
						attack = attack  + makeMove(b->fen,move , nextState);
					}
				}

				for(vector<string> v : pretendMoves){
					for(string move : v){
		        		pretendAttack = pretendAttack  + makeMove(p->fen,move,nextState);
					}
				}
				delete(p);
		        if (b->side_to_play == "b")
		        ...
		        //delete(b)
        return materialScore + mobilityScore + attackScore;		
		    }
		    else {
		        return rscore;
		    }
...
{
	string nextState;
	if( isGameOver(b->grid) || depth <= 0){
		return	evaluation(b);	
	}
	vector<vector<string>>moves = {b->lion(),b->zebra(),b->giraffe(),b->pawn(),b->superpawn()};
	for(vector<string> v : moves){
		for(string move : v){
			makeMove(b->fen,move , nextState);
	        int eval = -1 * minimax(new movepiece(nextState), depth - 1, -1 * beta, -1 * alpha);
	        if (eval >= beta)
	        {
				moves.clear();
				delete(b);
...
int main(){
	...
	for(int i = 0  ; i < N ; i++){
		cout<<minimax(new movepiece(lines[i]),4, alpha, beta);
		if(i != N-1){
			cout<<endl;			
		}
	}
